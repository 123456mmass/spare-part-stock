...
export const GET = withCors(async (request: Request) => {
  try {
      ...
      ...new Set(
        parsedMessages.flatMap(({ metadata }) =>
          isRecord(metadata) && Array.isArray(metadata.pendingActionIds)
            ? metadata.pendingActionIds.filter(
    ...
    console.error("Mobile AI chat history route error:", error);
    return NextResponse.json(
...
export const DELETE = withCors(async (request: Request) => {
  try {
    const user = await requireAuthFromRequest(request);
    const webLineUserId = `web:${user.id}`;
    ...
    console.error("Mobile AI chat history delete route error:", error);
    return NextResponse.json(
